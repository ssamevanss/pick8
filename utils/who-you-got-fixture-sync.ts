...
function deriveMatchdayStatus(
  ...
  if (statuses.every((status) => status === "scheduled" || status === "timed")) {
    return hasStartedFixture ? "scoring" : existingStatus === "open" ? "open" : "upcoming";
  }
...
export async function syncWhoYouGotFixtures(input: SyncInput): Promise<FixtureSyncSummary> {
  ...
  return diagnostics.stage("total", () => syncWhoYouGotFixturesInternal(input, diagnostics));
}

async function syncWhoYouGotFixturesInternal(input: SyncInput, diagnostics: ReturnType<typeof createSyncDiagnostics>): Promise<FixtureSyncSummary> {
  ...
  const applied = await diagnostics.stage("fixture_application", async () => {
