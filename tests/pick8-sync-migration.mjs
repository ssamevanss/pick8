...
create table public.seasons (id uuid primary key, name text, updated_at timestamptz default now());
...
insert into public.seasons(id,name) values ('${season}', '2026/27');
...
const clean = async () => db.exec(`update public.matchdays set sync_pending=false, scoring_pending=false;
...
await test('status changes retain sync/competition work without invalidating scorer input revisions', async () => {
  ...
  const after = await state();
  assert.equal(after.scoring_revision, before.scoring_revision);
  assert.equal(after.sync_pending, true);
