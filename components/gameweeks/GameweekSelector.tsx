...
export default function GameweekSelector({
  ...
  return (
    <div className="mb-4 grid grid-cols-[48px_1fr_48px] items-center rounded-2xl bg-slate-950 p-2 ring-1 ring-slate-800">
      {previousGameweek ? (
        <Link
          href={getGameweekHref(basePath, previousGameweek.id)}
          className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-900 text-lg font-bold text-slate-200"
          aria-label="Previous gameweek"
        >
          ←
        </Link>
      ) : (
        <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-900 text-lg font-bold text-slate-700">
          ←
        </span>
      )}
      ...
      <div className="px-2 text-center">
        ...
        {gameweeks.length > 0 ? (
          <select
            ...
            className="mt-1 max-w-full cursor-pointer appearance-none rounded-lg bg-slate-950 px-3 py-1 text-center font-semibold text-white outline-none ring-1 ring-transparent hover:bg-slate-900 focus:ring-emerald-500"
