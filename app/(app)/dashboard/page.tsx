...
export default async function DashboardPage() {
  ...
  return <div className="space-y-6">
    {currentMatchday && currentEntryUi && currentLifecycle ? <section className="brand-card border-emerald-300/30 bg-[linear-gradient(110deg,rgba(52,211,153,0.14),rgba(15,23,42,0.2))] p-4 sm:p-5"><div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center"><div className="min-w-0"><p className="brand-eyebrow">Current matchday · {season.name}</p><div className="mt-1 flex flex-wrap items-center gap-2"><h1 className="text-2xl font-black text-white sm:text-3xl">Matchday {currentMatchday.matchday_number}</h1><span className="brand-pill">{currentEntryUi.status}</span></div><p className="mt-2 font-bold text-emerald-100">{matchdayTiming(currentMatchday, now)}</p><p className="mt-1 text-sm text-slate-300">{currentEntryUi.detail}</p></div><div className="flex items-center justify-between gap-4 md:justify-end">{currentLifecycle === "Completed" && currentEntry?.calculated_score !== null && currentEntry?.calculated_score !== undefined ? <div><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Your score</p><p className="text-2xl font-black text-emerald-200">{currentEntry.calculated_score} pts</p></div> : null}<Link className="brand-button-primary w-full sm:w-auto" href={`/my-picks?matchday=${currentMatchday.matchday_number}`}>{currentEntryUi.cta}</Link></div></div></section> : <section className="brand-card p-5"><h1 className="text-xl font-black text-white">Current matchday</h1><p className="mt-2 text-sm text-slate-400">No matchdays are available for this season yet.</p></section>}
